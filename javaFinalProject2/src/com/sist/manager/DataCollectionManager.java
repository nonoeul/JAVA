...
//			private String key; //동영상
			...
			for (int i = 0; i < urls.length; i++) 
			{
				...
				for(int j=0;j<title.size();j++)
				{
					...
					vo.setSiner(singer.get(j).text());
					vo.setAlbum(album.get(j).text());
					vo.setPoster(poster.get(j).attr("src"));
					vo.setState(state);
					vo.setIdcrement(Integer.parseInt(id));
					vo.setKey(title.get(i).text());
					list.add(vo);
					k++;
				}
			}
	...
	public static String youtubeKeyData(String title)
	{
		...
		try {
			...
			Pattern p=Pattern.compile("/watch\\?v=[^가-힣]+");///watch\\?v= 뒤에 한글이 없는 .. 이라는 뜻
			Matcher m=p.matcher(data);
			while(m.find())
			{
				String s=m.group();
				s=s.substring(s.indexOf("=")+1,s.indexOf("\""));//=+1부터 "까지 잘라라
				key=s;
				break;
			}
