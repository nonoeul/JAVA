...
public class FindPanel extends JPanel implements ActionListener{ //JPanel=화면
	...
	public FindPanel()
	{
		...
		table.setRowHeight(35);// 검색해서 출력되는 사진 크기 (각 항목 크기) 출력 길이 설정
		//타이틀바 고정
		...
		for(GenieMusicVO vo:list)
		{
			...
			Object[] data= {new ImageIcon(img), vo.getTitle(),vo.getSiner(),vo.getAlbum()
		};
			model.addRow(data);
	...
	{
		for(int i=model.getRowCount()-1;i>=0;i--) //뒤에서(밑에서) 부터 지운다
		{
			model.removeRow(i);
		}
			try {
