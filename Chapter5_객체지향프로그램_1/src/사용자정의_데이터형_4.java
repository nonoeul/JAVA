...
public class 사용자정의_데이터형_4 {
	public static void main(String[] args) {
		...
		// 클래스데이터형 => 일반 데이터형과 동일
		// 한 메모리에 여러 개의 다른 데이터를 모아서 관리하는 과정
		// 클래스는 누가 더 데이터를 잘 관리하냐에 대한 것 
		Music[] music=new Music[50];
		// int[] arr=new int[100] => 사용자정의 데이터형 
		System.out.println(MusicDate.title.length);
		for (int i = 0; i < 50; i++) 
		{
			music[i]=new Music();
			music[i].title=MusicDate.title[i];
			music[i].singer=MusicDate.singer[i];
			music[i].album=MusicDate.album[i];
			music[i].state=MusicDate.state[i];
			music[i].modify=MusicDate.modify[i];
			// music의 값을 할당했다.
		}
		// Music => 목록 출력
